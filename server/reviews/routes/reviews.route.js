...
const executeConcurrent = cassandraDriver.concurrent.executeConcurrent;
...
router.get('/', async (req, res) => {
  try {
    // check to see if the cache is there
    const { product_id } = req.query;
    const cachedProduct = cache.get(product_id);
    if (cachedProduct) {
...
router.get('/meta/', async (req, res) => {
  const { product_id } = req.query;
  try {
    // check to see if the meta-data for product is cached
    const cachedProduct = cache.get(product_id+'meta');
    if (cachedProduct) {
      res.status(200).json({...cachedProduct});
    } else {
      const characteristicObj = { product_id: product_id };
      // Create concurrency level for getting ratings, recommends, and characteristics
      // based on product id
      const getRelevantMetaData = [
        { query: 'select rating from reviews.reviews where product_id = ?',
          params: [ product_id ] },
        { query: 'select recommend from reviews.reviews where product_id = ?',
          params: [ product_id ] },
      ];
      // create concurrent
      const concurrentData = await executeConcurrent(cassandraClient, getRelevantMetaData, { collectResults: true });
      const [ ratingsMeta, recommendMeta, characteristicsMeta ] = concurrentData.resultItems;
      ...
      for (let obj of ratingsMeta.rows) {
      ...
      // Iterate thru all the ratings and increment
      for (let obj of recommendMeta.rows) {
      ...
      await cassandraClient.stream(characteristcQuery, [product_id], { prepare: true })
        ...
        .on('end', async function () {
          ...
          cache.set(product_id+'meta', characteristicObj);
