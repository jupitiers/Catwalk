...
const RatingsAndReviews = () => {
  ...
  const { getReviewsByProductId, productId } = useContext(APIContext);
  ...
  return (
    <div className={styles.sectionContainer}>
      ...
      {reviews.length > 0 ? (
        <div className={styles.ratingsAndReviewsContainer}>
          ...
          <div className={styles.reviewActions}>
            {reviews.length > 2 && reviews.length > reviewsShowing && (
              <button onClick={showMoreReviews} className={styles.moreReviews}>
                More Reviews
              </button>
            )}
            <button onClick={openCreate} className={styles.reviewButton}>
              Add A Review +
            </button>
          </div>
        </div>
        ...
        <>
          <button onClick={openCreate} className={styles.firstReviewButton}>
